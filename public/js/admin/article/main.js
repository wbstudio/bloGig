$(function () {
  ...
  console.log(endActiveFlag); //公開開始時刻
  ...
  console.log(setEYear); //公開開始時間SET
  ...
  setReleaseDay();
  $('select[name=release_day] option[value=' + setRDate + ']').prop('selected', true); //公開終了時間SET
  ...
  $('select[name=end_day] option[value=' + setEDate + ']').prop('selected', true); // 年/月 変更--公開開始
...
function setReleaseDay() {
  ...
  monthVal = $('select[name=release_month]').val(); // 指定月の末日
...
function setEndDay(endActiveFlag) {
  ...
  monthVal = $('select[name=end_month]').val(); // 指定月の末日
